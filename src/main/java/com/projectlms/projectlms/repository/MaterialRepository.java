...
public interface MaterialRepository extends SoftDeletesRepository<Material, Long>{
    ...
    @Query(value = "SELECT * FROM M_MATERIAL m WHERE m.section_id = ?", nativeQuery = true)
    List<Material> searchAll(Long sectionId);

    @Query(value = "SELECT * FROM M_MATERIAL m WHERE m.id = ? AND m.section_id = ?", nativeQuery = true)
