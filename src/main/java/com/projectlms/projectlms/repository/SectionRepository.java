...
public interface SectionRepository extends SoftDeletesRepository<Section, Long> {
    ...
    @Query(value = "SELECT * FROM M_SECTION s WHERE s.deleted_at IS NULL AND s.course_id = ?", nativeQuery = true)
    List<Section> searchAll(Long courseId);

    @Query(value = "SELECT * FROM M_SECTION s WHERE s.deleted_at IS NULL AND s.id = ? AND s.course_id = ?", nativeQuery = true)
